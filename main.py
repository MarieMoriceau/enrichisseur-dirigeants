...
def calculer_salutation(prenom: str, email: str) -> tuple:
    """
    Retourne (prenom_fiable: bool, salutation: str)
    Règle : on ne met le prénom que si on peut le confirmer dans l'email.
    """
    import unicodedata, re

    EMAILS_GENERIQUES = {
        'contact','info','rh','commercial','hello','bonjour','direction',
        'accueil','admin','sales','office','support','service','equipe',
        'team','general','post','courrier','secretariat','agence'
    }
    ...
    # Email générique
    if local in EMAILS_GENERIQUES or any(local.startswith(g+"@") for g in EMAILS_GENERIQUES):
        return False, "Bonjour,"

    # Prénom ressemble à un nom de société (tout en majuscules > 2 chars)
    if prenom.isupper() and len(prenom) > 2:
        return False, "Bonjour,"

    prenom_norm = normaliser(prenom.split("-")[0])  # "Jean-Marc" → "jean"
    prenom_norm_full = normaliser(prenom)             # "jean-marc"

    # Découper la partie locale sur . - _ et chiffres
    morceaux = [m for m in re.split(r"[.\-_0-9]+", local) if m]

    fiable = False

    # Condition 1 : prénom normalisé == un morceau exact
    if prenom_norm in morceaux or prenom_norm_full in morceaux:
        fiable = True

    # Condition 2 : prénom ≥ 4 lettres et un morceau commence par le prénom
    # ex: "alexandre" vs morceau "alex" → non fiable
    # ex: "alex" vs morceau "alexandre" → oui (morceau commence par le prénom)
    if not fiable and len(prenom_norm) >= 4:
        if any(m.startswith(prenom_norm) or prenom_norm.startswith(m) and len(m) >= 4 for m in morceaux):
            fiable = True

    # Condition 3 : prénom ≥ 4 lettres apparaît dans la partie locale
    if not fiable and len(prenom_norm) >= 4:
        if prenom_norm in local or prenom_norm_full in local:
            fiable = True

    # Condition 4 : initiales composées (ex: Anne-Marie → "am" dans email)
    if not fiable and "-" in prenom:
        parties = [normaliser(p) for p in prenom.split("-") if p]
        if len(parties) >= 2:
            initiales = "".join(p[0] for p in parties)  # "am" pour Anne-Marie
            if any(m.startswith(initiales) for m in morceaux):
                fiable = True

    if fiable:
        # Capitaliser correctement (garde les tirets)
        sal_prenom = "-".join(p.capitalize() for p in prenom.split("-"))
        return True, f"Bonjour {sal_prenom},"
    else:
        return False, "Bonjour,"
...
def generer_excel(rows: list) -> bytes:
    ...
    headers  = ['Organisation','Prénom','Nom','Titre','Email','Téléphone','LinkedIn','Domaine','Confiance','Source','Dans Pipedrive','Prénom fiable','Salutation']
    col_map  = ['societe','prenom','nom_dg','titre','email','phone','linkedin','domaine','confiance','source','dans_pipedrive','prenom_fiable','salutation']
    thin     = Side(style='thin', color="e2e8f0")
    border   = Border(left=thin, right=thin, top=thin, bottom=thin)
    ...
    # Calcul salutation pour chaque contact
    for r in rows:
        fiable, salutation = calculer_salutation(r.get('prenom',''), r.get('email',''))
        r['prenom_fiable'] = 'oui' if fiable else 'non'
    ...
    for row_idx, row in enumerate(rows, 4):
        ...
        for col_idx, key in enumerate(col_map, 1):
            ...
            elif key == 'prenom_fiable':
                if val == 'oui':
                    c.fill = PatternFill('solid', start_color="dcfce7")
                    c.font = Font(name='Arial', size=9, color="166534", bold=True)
                elif val == 'non':
                    c.fill = PatternFill('solid', start_color="fee2e2")
                    c.font = Font(name='Arial', size=9, color="991b1b", bold=True)
                else:
                    c.fill = PatternFill('solid', start_color=bg)
            elif key == 'salutation':
                c.font = Font(name='Arial', size=9, color="1e3a5f", bold=True if 'Bonjour,' != val else False)
                c.fill = PatternFill('solid', start_color=bg)
            else:
                c.fill = PatternFill('solid', start_color=bg)

    for i, w in enumerate([22,14,18,28,32,16,14,20,12,22,28,14,22], 1):
        ws.column_dimensions[get_column_letter(i)].width = w
